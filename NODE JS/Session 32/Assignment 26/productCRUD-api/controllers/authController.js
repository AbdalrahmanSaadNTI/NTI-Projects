...
const signin = (req, res) => {
    ...
    if (!req.body.userName || !req.body.password) {
        return res.status(400).json({ login: 'failed, username and password required' });
    }
            ...
            if (users.length == 0) {
                ...
                bcrypt.compare(req.body.password, users[0].password, function (err, result) {
                    ...
                    if (result) {
                        ...
                        return res.status(200).json({ login: 'done', token: token, role: users[0].role, fullName: users[0].fullName, userId: users[0]._id });
                    }
                    else {
